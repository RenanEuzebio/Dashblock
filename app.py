...
SAFE_VERSION = re.compile(r"^[0-9]+(\.[0-9]+){1,2}$")
...
# ---- Persistent SSH connection (faster) ----
    ...
    with _conn_lock:
        if _conn_singleton is None:
            ...
            _conn_singleton = Connection(
                ...
                connect_timeout=20, connect_kwargs={"key_filename": str(SSH_KEY_PATH)}
            )
...
def get_game_versions() -> List[Dict[str, object]]:
    with urllib.request.urlopen("https://meta.fabricmc.net/v2/versions/game", timeout=15) as r:
        return json.load(r)

def resolve_loader_version(mc: str) -> str:
    with urllib.request.urlopen(f"https://meta.fabricmc.net/v2/versions/loader/{mc}", timeout=15) as r:
        d = json.load(r)
    if not d: raise RuntimeError(f"No Fabric loader for {mc}")
    return d[0]["loader"]["version"]

def resolve_installer_version() -> str:
    with urllib.request.urlopen("https://meta.fabricmc.net/v2/versions/installer", timeout=15) as r:
        d = json.load(r)
...
def install_prereqs(pm: Optional[str], java_pkg: str) -> None:
    if pm=="apt":
        _run("sudo apt-get update -y && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y curl ca-certificates " + java_pkg)
    elif pm=="dnf":
        _run("sudo dnf -y install curl ca-certificates " + java_pkg)

# ---- FS & meta on VPS ----
def ensure_tree() -> None:
    _run(f'mkdir -p "$HOME/{WORLDS_DIR}"')

def list_world_names() -> List[str]:
    out = _run(f'cd "$HOME/{DASHBLOCK_DIR}" 2>/dev/null || mkdir -p "$HOME/{DASHBLOCK_DIR}"; [ -d "$HOME/worlds" ] || true; '
               f'[ -d "$HOME/{WORLDS_DIR}" ] && find "$HOME/{WORLDS_DIR}" -maxdepth 1 -mindepth 1 -type d -printf "%f\n" || true')
    return [x for x in out.splitlines() if x]

def ensure_world(name: str) -> None:
    if not SAFE_NAME.fullmatch(name): raise HTTPException(400, "Invalid world name.")
    _run(f'''
base="$HOME/{WORLDS_DIR}/{name}"; mkdir -p "$base/mods"
...
def setup_fabric(name: str, mc: str, mem_gb: int, motd: str, online: bool) -> Dict[str,str]:
    if not SAFE_VERSION.fullmatch(mc): raise HTTPException(400,"mc_version must look like 1.20.1 or 1.21")
    ...
    jar = f"https://meta.fabricmc.net/v2/versions/loader/{mc}/{loader}/{inst}/server/jar"
    _run(f'cd {bpath(name)} && curl -fsSL "{jar}" -o fabric-server-launch.jar && printf "eula=true\\n" > eula.txt')
...
def read_console(name: str, tail: int=200) -> List[str]:
    out=_run(f'cd {bpath(name)} && (test -f server.log && tail -n {int(tail)} server.log) || echo ""'); return out.splitlines()
...
class ProxyManager:
    ...
    def publish(self, world:str, remote_port:int)->int:
        fut=asyncio.run_coroutine_threadsafe(self._pub(world,int(remote_port)), self._loop)
        return fut.result()
    ...
    def unpublish(self, world:str)->bool:
        fut=asyncio.run_coroutine_threadsafe(self._unpub(world), self._loop)
        return fut.result()
...
def api_setup(name:str, mc_version:str, mem_gb:int, motd:str="A Minecraft Server", online_mode:str|bool="true")->dict:
    try:
        if not SAFE_NAME.fullmatch(name): raise HTTPException(400,"Invalid world name.")
        ensure_world(name)
        meta=read_meta(name); updated="full" if meta.get("mc_version")!=mc_version else "settings"
        info = setup_fabric(name, mc_version, int(mem_gb), motd, _parse_bool(online_mode)) if updated=="full" else update_settings(name, int(mem_gb), motd, _parse_bool(online_mode))
        rp=read_port(name); pub=proxy.publish(name, rp)
        return {"ok":True,"world":name,"updated":updated,"setup":info,"config":read_props(name),"mods":list_mods(name),"meta":read_meta(name),
...
def api_console(name:str, tail:int=200)->dict:
    try: ensure_world(name); return {"ok":True,"world":name,"lines": read_console(name, tail=int(tail))}
    except Exception as e: raise HTTPException(502, f"SSH error: {e}")

# ---- CORS & App ----
app = Litestar(
    route_handlers=[api_worlds, api_create, api_delete, api_details, api_setup, api_start, api_stop, api_versions, api_status, api_console],
